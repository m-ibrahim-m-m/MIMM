...
def process_data(data):
    ...
    def determine_status(row):
        ...
        mapping = {'CNCL':'Canceled', 'CNF':'Completed', 'JIPR':'In Progress', 'NCMP':'Not Executed & Deleted'}
...
def create_filters(data):
    ...
    with st.sidebar.expander("🏭 Plants", expanded=True):
        plants = st.multiselect("Plants", options=sorted(data['Plant'].dropna().unique()),
                                default=sorted(data['Plant'].dropna().unique())[:2], label_visibility="collapsed")
    with st.sidebar.expander("📅 Time Range", expanded=True):
        years = st.slider("Years", min_value=int(data['Year'].min()), max_value=int(data['Year'].max()),
                          value=(int(data['Year'].min()), int(data['Year'].max())), label_visibility="collapsed")
        available_months = sorted(data['Month'].dropna().unique(),
                                  key=lambda x: MONTH_ORDER.index(x) if x in MONTH_ORDER else 99)
        months = st.multiselect("Months", options=available_months, default=available_months, label_visibility="collapsed")
    with st.sidebar.expander("⚙️ Order Attributes", expanded=False):
        plan_type = st.multiselect("Plan Type", options=data['Plan Type'].unique(), default=data['Plan Type'].unique())
        statuses = st.multiselect("Order Status", options=data['Order Status'].unique(), default=data['Order Status'].unique())
        order_types = st.multiselect("Work Order Type", options=sorted(data['Order Type'].dropna().unique()),
                                     default=sorted(data['Order Type'].dropna().unique()))
    with st.sidebar.expander("🏗️ Department / Group", expanded=False):
        work_centers = st.multiselect("Main Work Center", options=sorted(data['Main Work Center'].dropna().unique()),
                                      default=sorted(data['Main Work Center'].dropna().unique()))
        groups = st.multiselect("Task List Code (Group)", options=sorted(data['Group'].dropna().unique()),
...
def plot_completion_donuts(fd):
    ...
    def donut(vals, labels, title, hit_color):
        ...
        fig.update_layout(title_text=title, showlegend=True,
                          legend=dict(orientation="h", y=-0.12, x=0.5, xanchor="center", bgcolor="rgba(0,0,0,0)", bordercolor="rgba(255,255,255,0.08)", borderwidth=1),
                          paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)",
                          font=dict(family="'IBM Plex Sans', sans-serif", color="#E2E8F0", size=12),
                          margin=dict(l=16, r=16, t=48, b=16))
        return fig
...
def plot_order_type_analysis(fd):
    ...
    fig = go.Figure()
    fig.add_bar(x=cd['Order Type'], y=cd['Planned'], name='Planned', marker_color=COLORS["planned"], marker_line_width=0, opacity=0.85)
    fig.add_bar(x=cd['Order Type'], y=cd['Actual'], name='Actual', marker_color=COLORS["accent"], marker_line_width=0, opacity=0.85)
    apply_layout(fig)
...
def show_raw_data(fd):
    ...
    st.dataframe(display_df.sort_values('Basic start date', ascending=False), width="stretch", height=380)
