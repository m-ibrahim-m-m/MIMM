...
# ========================= CHARTS (Fixed with width="stretch" + theme=None) =========================
    ...
    with col1:
        ...
        if total_p:
            st.plotly_chart(donut([comp_p, total_p-comp_p], ['Completed','Remaining'],
                                  "Planned Orders Completion", COLORS["success"]), width="stretch", theme=None)
        st.markdown('</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="chart-card">', unsafe_allow_html=True)
        if total_o:
            st.plotly_chart(donut([comp_o, total_o-comp_o], ['Completed','Remaining'],
                                  "Overall Orders Completion", COLORS["accent"]), width="stretch", theme=None)
        st.markdown('</div>', unsafe_allow_html=True)
